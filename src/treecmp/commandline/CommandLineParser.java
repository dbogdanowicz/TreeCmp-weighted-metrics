...
public class CommandLineParser {
    ...
    private final static String SS_DESC = "- Report normalized values.";
    private final static String II_DESC = "- Include summary section in the output file.";
    private final static String E_DESC = "- Report extreme values only but in detailed manner.";
   ...
   // private final static String F_DESC = "- Use MS/MC metrics for trees with free leaf set. ";
    ...
    private final static String H_DESC = "- Super tree huristic mode.";
    private final static String H_ARG = "serachType: 0|1 startTreeNum |2 startTreeFile, 0 - random, 1 - chosen tree from the input file as a start tree, 2 - custom start tree from the file.";
    ...
    private final static String NN_DESC = "- ARGs mode.";
    private final static String NN_ARG = "size";
    
    private final static String CMD_LINE_SYNTAX = "java -jar TreeCmp.jar -s|-w <size>|-m|-r <refTreeFile>|-h <type> <startTreeFile> "
            +" -d <metrics> -i <inputFile> -o <outputFile> [-N] [-P] [-I] [-A|-O] [-E]\n"
    ...
    public static Command run(String args[]) {
        ...
        Option oH = new Option("h", H_DESC);
        oH.setArgName(H_ARG);
        oH.setValueSeparator(' ');
        oH.setArgs(2);
        
        Option oNn = new Option("n", NN_DESC);
        oNn.setArgName(NN_ARG);
        oNn.setArgs(1);
        ...
        cmdOpts.addOption(oH);
        cmdOpts.addOption(oNn);
      ...
      //  customMOpts.addOption(oF);
        ...
        try {
            ...
            if (commandLine != null) {
                ...
                if (commandLine.hasOption(oE.getOpt())) {
                    IOset.setExtreme(true);
                    custOpts.add(oE);
                }
                /*
                ...
                */
                ...
                }else if (commandLine.hasOption(oH.getOpt())) {
                  
                   
                    List<Metric> metricList = ActiveMetricsSet.getActiveMetricsSet().getActiveMetrics();
                    String heuristicName = null;
                    if (!metricList.isEmpty()){
                        heuristicName = metricList.get(0).getCommandLineName();
                    }
                    String [] superTreeParams = commandLine.getOptionValues(oH.getOpt());
                    
                    int type = 0;
                    if (superTreeParams.length > 0){
                          String searchType = superTreeParams[0];
                          type = Integer.parseInt(searchType);
                    }
                    
                    StartTreeType startTreeType = null;
                    String startTreeFileName = null;
                    int startTreeNum = 1;
                    switch (type) {
                        case 0:
                            startTreeType = StartTreeType.RANDOM;
                            break;
                        case 1:
                            startTreeType = StartTreeType.TREE_FROM_DATA;
                            if (superTreeParams.length > 1) {
                                startTreeNum = Integer.parseInt(superTreeParams[1]);
                            }
                            break;
                        case 2:
                            startTreeType = StartTreeType.CUSTOM;
                            startTreeFileName = superTreeParams[1];
                            break;
                        case 3:
                            startTreeType = StartTreeType.CREATE_TREE_FILE;
                            startTreeFileName = superTreeParams[1];
                            break;
                        default:
                            throw new IllegalArgumentException("Unknown supertree search type!");
                    }

                    cmd = new RunSuperTreeCommand(0, "-h", heuristicName, startTreeType, startTreeFileName, startTreeNum);
                    analysisType=" super tree heuristic mode (-h)";
                    
                } else if (commandLine.hasOption(oNn.getOpt())) {
                    String sSpanSize = (String) commandLine.getOptionValue(oNn.getOpt());
                    int iSpanSize = Integer.parseInt(sSpanSize);
                    cmd = new RunNCommand(1, "-n", iSpanSize);
                    analysisType="ARGs comparison mode (-n) with span size: "+iSpanSize;    
                }  else{
...
class OptOrder implements Comparator {
    ...
    public OptOrder() {
        int i = 1;
        order.put("s", new Integer(i++));
        order.put("w", new Integer(i++));
        order.put("m", new Integer(i++));
        order.put("r", new Integer(i++));
        order.put("n", new Integer(i++));
        order.put("h", new Integer(i++));
        order.put("d", new Integer(i++));
        order.put("i", new Integer(i++));
        order.put("o", new Integer(i++));
        order.put("N", new Integer(i++));
        order.put("P", new Integer(i++));
        order.put("I", new Integer(i++));
        order.put("A", new Integer(i++));
        order.put("O", new Integer(i++));
        order.put("E", new Integer(i++));
